...
public interface SpendingRepository extends JpaRepository<Spending,Long> {
    ...
    @Query("SELECT   new com.example.DISI.DTO.GraphData(s.reason , sum(s.amount), count(s.spendingID) )from Spending s where  s.makingDate between ?1 and ?2 GROUP BY s.reason")
...
//    List<GraphData> findByMakingDateBetweenSum(LocalDate startDate, LocalDate endDate);
}
